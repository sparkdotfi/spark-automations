...
describe('MetaMorpho', function () {
    ...
    const susdeMarket = {
        ...
        oracle: '0x5D916980D5Ae1737a8330Bf24dF812b2911Aae25', // SUSDE ORACLE
        irm: '0x870aC11D48B15DB9a138Cf899d20F13F79Ba00BC', // DEFAULT IRM
        lltv: '860000000000000000', // 0.86
    }

    const usdeMarket = {
        loanToken: '0x6B175474E89094C44Da98b954EedeAC495271d0F', // DAI
        collateralToken: '0x4c9EDD5852cd905f086C759E8383e09bff1E68B3', // USDE
        oracle: '0xaE4750d0813B5E37A51f7629beedd72AF1f9cA35', // USDE ORACLE
        irm: '0x870aC11D48B15DB9a138Cf899d20F13F79Ba00BC', // DEFAULT IRM
        lltv: '860000000000000000', // 0.86
    }
    ...
    before(async () => {
        ...
        susdeMarketId = calculateMetaMorphoMarketId(
            susdeMarket.loanToken,
            susdeMarket.collateralToken,
            susdeMarket.oracle,
            susdeMarket.irm,
            susdeMarket.lltv,
        )
        usdeMarketId = calculateMetaMorphoMarketId(
            usdeMarket.loanToken,
            usdeMarket.collateralToken,
            usdeMarket.oracle,
            usdeMarket.irm,
            usdeMarket.lltv,
        )
    ...
    it('no pending caps', async () => {
        const { result } = await metaMorphoW3F.run('onRun')
    ...
    it('no pending caps (waiting for the timelock)', async () => {
        ...
        await metaMorphoVault.submitCap(Object.values(susdeMarket), susdeCurrentCap.add(1))
    ...
    it('one cap update to be accepted', async () => {
        ...
        await metaMorphoVault.submitCap(Object.values(susdeMarket), susdeInitialCap.add(1))
        await mine(2, { interval: timelock.sub(1) })

        const { result: negativeResult } = await metaMorphoW3F.run('onRun')
        ...
        const { result: positiveResult } = await metaMorphoW3F.run('onRun')
        ...
        expect(callData).to.deep.equal([
            {
                ...
                data: metaMorphoVault.interface.encodeFunctionData('acceptCap', [Object.values(susdeMarket)]),
            },
        ])
    ...
    it('two cap updates to be accepted', async () => {
        ...
        await metaMorphoVault.submitCap(Object.values(susdeMarket), susdeInitialCap.add(1))
        await metaMorphoVault.submitCap(Object.values(usdeMarket), usdeInitialCap.add(1))
        ...
        expect(callData).to.deep.equal([
            ...
            {
                to: addresses.mainnet.metaMorpho,
                data: metaMorphoVault.interface.encodeFunctionData('acceptCap', [Object.values(usdeMarket)]),
            },
        ])
